    ...
    <div id="templatemo_content">
        ...
        <div class="section_w670">
            ...
            <div class="div-a">工号：<br/>
                <c:forEach   items="${MStaff}" var="listMoreStaff">
                    ${listMoreStaff.sno} <br/>
                </c:forEach>
            </div>
            <div class="div-a">姓名：<br/>
                <c:forEach   items="${MStaff}" var="listMoreStaff">
                    ${listMoreStaff.sname} <br/>
                </c:forEach>
            </div>
            <div class="div-a">性别：<br/>
                <c:forEach   items="${MStaff}" var="listMoreStaff">
                    ${listMoreStaff.ssex} <br/>
                </c:forEach>
            </div>
            <div class="div-a">年龄：<br/>
                <c:forEach   items="${MStaff}" var="listMoreStaff">
                    ${listMoreStaff.sage} <br/>
                </c:forEach>
            </div>
            <div class="div-a">学历：<br/>
                <c:forEach   items="${MStaff}" var="listMoreStaff">
                    ${listMoreStaff.sedu} <br/>
                </c:forEach>
            </div>
            <div class="div-a">电话：<br/>
                <c:forEach   items="${MStaff}" var="listMoreStaff">
                    ${listMoreStaff.stel} <br/>
                </c:forEach>
            </div>
            <div class="div-a">邮箱：<br/>
                <c:forEach   items="${MStaff}" var="listMoreStaff">
                    ${listMoreStaff.semail} <br/>
                </c:forEach>
            </div>
            <div class="div-a">职位：<br/>
                <c:forEach   items="${MStaff}" var="listMoreStaff">
                    ${listMoreStaff.sposition} <br/>
                </c:forEach>
            </div>
            <div class="div-a">部门号：<br/>
                <c:forEach   items="${MStaff}" var="listMoreStaff">
                    ${listMoreStaff.dno} <br/>
                </c:forEach>
            </div>
            <div class="div-d">
                <br/><br/><br/>
